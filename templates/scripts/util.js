...
function addPostListener(callback, formId = 'mainForm', errorId = 'error') {
  ...
  form.addEventListener('submit', async(event) => {
    ...
    const originalTexts = [ ...submitButtons ].map(btn => btn.textContent);
    for (const [ index, btn ] of submitButtons.entries()) {
      btn.disabled = true;
      btn.textContent = btn.textContent.includes('Loading') ? btn.textContent : 'Loading...';
    }
    ...
    try {
      ...
      // Restore button states on success (before redirect)
      for (const [ index, btn ] of submitButtons.entries()) {
        btn.disabled = false;
